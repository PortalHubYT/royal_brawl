...
from input import sanitize_less_strict, sanitize_very_strict

class ShopItem():
        def __init__(self, session, name, price, icon, index, mob_type="zombie", more_nbt=""):
...
class Component(ApplicationSession):
    ...
    async def spawn_shop(self):
        self.shop = [ 
            ...
            ShopItem( self, "skeleton", 50, "minecraft:iron_sword", 1, "skeleton", more_nbt = """HandItems:[{id:"minecraft:bow",Count:1b},{}]"""),
            ShopItem( self, "diamond sword", 100, "minecraft:diamond_sword", 2, more_nbt = """HandItems:[{id:"minecraft:bow",Count:1b},{}]"""),
    ...
    async def onJoin(self, details):
        ...
        self.subscribe(self.refresh_shop, "game.tick")
        

    async def spawn_mob(self, player_data):
        mob_uid = str(uuid.uuid4())[:8]
        # nbt = get_nbt(player_data['display_name'], "122", mob_uid)
        nbt = get_nbt(player_data['display_name'], player_data['channel_id'], mob_uid, player_data['more_nbt'])
